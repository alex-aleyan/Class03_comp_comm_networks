...
int main (int argc, char **argv)
{
    ...
    for(current_line=0, test=-1; current_line<line_count; current_line++)
    {
        ...
        */
        ...
        memcpy(app_header_n_data+(sizeof(file_x_app_layer_t)), text_line[current_line], strlen(text_line[current_line]) );
        printBytes(app_header_n_data, sizeof(file_x_app_layer_t) + strlen(text_line[current_line]));


        //SEND THE DATA: TX Socket, TX Data, TX Data Size, flags, RX Socket Info (DOMAIN, IP and PORT), size of RX Address Struct)
        test=sendto( tx_socket_fd, 
                     app_header_n_data,                                       \
                     strlen(text_line[current_line])+sizeof(file_x_app_layer_t),    \
                     0,                                                             \
                     (struct sockaddr *) &tx_to_address,                            \
                     sizeof(tx_to_address)                                          );
